...
class PauseTaskParams {
  const PauseTaskParams({required this.task, required this.now});

  final Task task;
  final DateTime now;
}

/// Met l'exécution d'une tâche en pause.
///
/// Utilise [ExecutionEngine] pour calculer le nouvel état puis persiste la
/// tâche via [UpdateTask].
class PauseTask implements UseCase<void, PauseTaskParams> {
  const PauseTask(this._engine, this._updateTask);

  final ExecutionEngine _engine;
  final UpdateTask _updateTask;

  @override
  Future<Result<void>> call(PauseTaskParams params) async {
